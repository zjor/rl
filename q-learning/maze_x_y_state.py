"""
Solving the maze problem with
    - DQN
    - experience replay
    - target network
The state is represented as (x, y) pair.
The network has hidden layer.
"""
...
class Agent(AbstractAgent):
    ...
    def _predict_q_policy(self, state):
        w = self.env.width
        x, y = state % w, state // w
        z = torch.tensor([x, y], dtype=torch.float)
        return self.nn(z)

    def _predict_q_target(self, state):
        w = self.env.width
        x, y = state % w, state // w
        z = torch.tensor([x, y], dtype=torch.float)
        return self.target_nn(z)
